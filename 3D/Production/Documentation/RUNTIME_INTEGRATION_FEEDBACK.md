...
# Device finding: incomplete lake boats and diorama depth

- USD hierarchy inspection traced the apparent “tree in the water” to `midground/sailboat` and `sailboat_001`: their masts render, but hull/sail silhouettes do not read from `camera_gameplay`. Runtime hides both objects. Re-export complete low-poly boats with verified materials and transforms, then provide a slow seamless bob/drift clip in `lax_arena_ambient`.
- Runtime now enables RealityKit's native depth-of-field effect. Stage `camera_gameplay`, foreground framing, gameplay field, lake, and far shore at clearly separated camera-space depths. Do not pre-blur the goal, playable field, shooter, goalie, ball, or trajectory path. Keep foreground foliage close enough and the far shore distant enough for the native focus falloff to read.
