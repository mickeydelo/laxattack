...
_SRGB = {   # concept colours (sRGB) -> linear shader values
    ...
    "helmet_cyan": ((0.19215686274509805, 0.8470588235294118, 1.0), 0.22, 0.5), "cage_white": ((0.95, 0.95, 0.93), 0.2, 0.3), "kit_cream_v9": ((0.99, 0.96, 0.92), 0.55, 0.0),
    "kit_red_v9": ((1.0, 0.35294117647058826, 0.23529411764705882), 0.45, 0.1), "kit_navy_v9": ((0.050980392156862744, 0.16862745098039217, 0.3215686274509804), 0.45, 0.1), "kit_cyan_v9": ((0.19215686274509805, 0.8470588235294118, 1.0), 0.40, 0.1),
    ...
    "grip_dark_v9": ((0.12, 0.13, 0.16), 0.75, 0.0), "cord_tan_v9": ((0.74, 0.57, 0.38), 0.75, 0.0), "cord_tan_dark_v9": ((0.52, 0.38, 0.25), 0.8, 0.0),
}
...
def curl_cluster(Hb, c, r, mat, n, rc, seed):
    rnd = random.Random(seed); Hb.add(ellipsoid(tuple(c), (r * 0.9, r * 0.9, r * 0.9), 14, 10), mat, "head")
    g = math.pi * (3 - math.sqrt(5))
    for i in range(n):
        z = 1 - 2 * (i + 0.5) / n; rr = math.sqrt(max(0.0, 1 - z * z)); t = g * i
        p = V(c) + V((math.cos(t) * rr, math.sin(t) * rr, z)) * r; k = rc * rnd.uniform(0.85, 1.2)
        Hb.add(ellipsoid(tuple(p), (k, k, k), 8, 6), mat, "head")

def v9_hair_puffs(s, H, Hb):          # Mina: centre-part cap + two big curly puffs with headbands
    bnd = lambda az: _interp([(0, 30), (14, 26), (34, 15), (56, 6), (80, 2), (100, -14), (130, -32), (180, -40)], az)
    cap, rim = hair_cap(H, bnd, base=1.05, grooves=22, depth=0.030, part=True, back_bulge=0.04, pole=H.point(180, 10, 1.1))
    Hb.add(cap, s["hair"], "head"); Hb.add(sweep(rim, [0.011] * len(rim), 8, 1.0, cap0=False, cap1=False), s["hair"], "head")
    for sx in (1, -1):
        c = V(H.point(62 * sx, 46, 1.38))
        curl_cluster(Hb, c, 0.13, s["hair"], 70, 0.034, 40 + (sx > 0))
        base = V(H.point(62 * sx, 40, 1.10))
        Hb.add(torus(tuple(base), 0.07, 0.014, 22, 8, "Z"), s.get("goggle_accent", "tie_cream"), "head")
        pts = [H.point(60 * sx, 16, 1.06), H.point(68 * sx, -4, 1.07), H.point(70 * sx, -24, 1.065), H.point(66 * sx, -40, 1.055)]
        Hb.add(sweep(pts, [0.028, 0.028, 0.02, 0.004], 12, 0.5), s["hair"], "head")

def v9_hair_helmet_curls(s, H, Hb):   # Ollie: short cap under the helmet + curls peeking out at the rim
    bnd = lambda az: _interp([(0, 32), (40, 22), (70, 2), (110, -26), (180, -40)], az)
    cap, rim = hair_cap(H, bnd, base=1.03, grooves=18, depth=0.008, part=False, back_bulge=0.0)
    Hb.add(cap, s["hair"], "head")
    rnd = random.Random(55)
    for az in list(range(-176, -60, 9)) + list(range(64, 180, 9)):
        el = bnd(az) + rnd.uniform(-2, 6)
        k = rnd.uniform(0.026, 0.034); Hb.add(ellipsoid(tuple(H.point(az, el, 1.09)), (k, k, k), 8, 6), s["hair"], "head")
        k = rnd.uniform(0.022, 0.03); Hb.add(ellipsoid(tuple(H.point(az + 4, el + 7, 1.10)), (k, k, k), 8, 6), s["hair"], "head")
    for az in range(-28, 29, 8):
        k = rnd.uniform(0.02, 0.026); Hb.add(ellipsoid(tuple(H.point(az, 31 + rnd.uniform(-1, 2), 1.075)), (k, k, k), 8, 6), s["hair"], "head")
...
def v9_goggles(s, H, G):
    ...
    for sx in (1, -1):
        G.add(superellipsoid((0.014, 0.024, 0.030), 0.45, 0.45, 10, 6, gp(48 * sx, -4, 0.022)), s.get("goggle_accent", "goggle_white"), "head")
...
def v9_helmet(s, H, G):
    ...
    for i in range(useg):
        a = vseg * useg + i; b = vseg * useg + (i + 1) % useg; faces.append((a, b, top))
    SHELL, STRIPE = s.get("helmet_shell", "helmet_navy_v9"), s.get("helmet_stripe", "helmet_cyan")
    G.add((verts, faces), SHELL, "head")
    rim = [H.point(az, bnd(az) + 0.5, SH + 0.005) for az in range(-180, 181, 5)]
    G.add(sweep(rim, [0.017] * len(rim), 8, 1.0, cap0=False, cap1=False), SHELL, "head")
    ...
    def band(az_c, el0, el1, w=12.0, nu=8, nv=18):          # raised strip hugging the shell, edges tapered into it
        ...
        return verts, faces
    G.add(band(0, 24, 88), STRIPE, "head"); G.add(band(180, 88, -26), STRIPE, "head")
    ...
    for sx in (1, -1):
        G.add(_oriented(H, 72 * sx, 4, (0.034, 0.014, 0.062), (shell_sc(72 * sx, 4) - 1.0) * H.r.z + 0.010), STRIPE, "head")
    ...
    G.add(sweep(chin, [0.032] * len(chin), 12, 0.6), SHELL, "head")
    for k_ in (-1, 0, 1):
        G.add(_oriented(H, 14 * k_, -52, (0.006, 0.006, 0.014), 0.05), "helmet_vent", "head")
    G.add(sweep(lip, [0.022] * len(lip), 10, 1.0), SHELL, "head")
    def cp(az, el, sc):
...
def build_character_v9(s, collection, arm, look):
    ...
    for k in ("face", "hair", "headgear") + (("chest_protector",) if s.get("goalie_pads", look == "goalie") else ()):
        o = parts.pop(k, None)
        if o is not None:
    ...
    try:
        ...
        for side, sx in (("L", 1.0), ("R", -1.0)):
            ...
            if not s.get("glove"):     # chunky toy hand: palm, four fingers, thumb
        ...
        clear_proportions()
    if not s.get("glove") and "gloves" in parts:
        bpy.data.objects.remove(parts.pop("gloves"), do_unlink=True)
    if s.get("goalie_pads", look == "goalie"):        # rounded protector pads under the jersey + front number
    ...
    if look == "player":
        vs = [o.matrix_world @ v.co for o in parts.values() if o.type == "MESH" for v in o.data.vertices
              if any(m and m.name == "M_" + s["kit"] for m in o.data.materials)]
